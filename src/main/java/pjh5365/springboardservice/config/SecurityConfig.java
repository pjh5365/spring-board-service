...
import pjh5365.springboardservice.auth.JwtFilter;
...
public class SecurityConfig {
    ...
    public SecurityConfig(JwtProvider jwtProvider, /*CustomAuthenticationProvider authenticationProvider,*/
            AuthenticationConfiguration authenticationConfiguration, MemberRepository memberRepository) {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()    // 인가 요청이 오면
                .antMatchers("/api/register", "/", "/login").permitAll()   // 회원가입 및 로그인 api 는 누구나 사용가능하게 설정 (이 설정이 권한인증설정 보다 앞에 있어야 정상적으로 작동함)
        ...
        // ;
                .addFilterBefore(new JwtFilter(jwtProvider, memberRepository), JwtAuthenticationFilter.class)
                .addFilterAt(new JwtAuthenticationFilter(authenticationManager(authenticationConfiguration), jwtProvider), UsernamePasswordAuthenticationFilter.class); // JwtAuthenticationFilter 를 UsernamePasswordAuthenticationFilter 전에 사용
